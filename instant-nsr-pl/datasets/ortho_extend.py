...
def load_per_prediction(root_dir, test_object, imSize, view_types, load_color=False, cam_pose_dir=None,
                      normal_system='front', erode_mask=True, camera_type='ortho', cam_params=None):
    """
    Load predictions from
    root_dir: directory for root
    test_object: directory for subject, e.g.: owl
    imSize: config中指定的render分辨率，默认使用PIL resize
    view_types: 输入视角方向
    ...
    for idx, view in enumerate(view_types):
        ...
        # 读取生成的rgb, 提取mask
        color_mask = np.array(
            PIL.Image.open(os.path.join(root_dir, test_object, 'masked_colors/rgb_000_%s.png' % (view))).resize(
                imSize))[:, :, 3]
        # (optional)优化mask
        ...
        # 获得pose
        all_poses.append(inv_RT(RT_cv))  # opencv cam2world (3, 4)
        all_w2cs.append(RT_cv)  # opencv W2C (3,4)

        # whether to 
        normal_cam_cv = normal_opengl2opencv(normal)  # convert normal from opengl to opencv
    ...
    return np.stack(all_images), np.stack(all_masks), np.stack(all_normals), \
        np.stack(all_normals_world), np.stack(all_poses), np.stack(all_w2cs), np.stack(ray_origins), np.stack(
        directions), np.stack(all_color_masks)
