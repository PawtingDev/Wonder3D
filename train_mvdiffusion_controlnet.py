...
def log_validation(dataloader, vae, feature_extractor, image_encoder, unet, cfg: TrainingConfig, accelerator,
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.autocast("cuda"):
            ...
            for guidance_scale in cfg.validation_guidance_scales:
                ...
                # out_rgb, out_normal = out[:num_images // 2], out[num_images // 2:]
                # out = []
                # for ii in range(num_images // 2):
                #     out.append(out_rgb[ii])
                #     out.append(out_normal[ii])
    ...
    images_control_all = torch.cat(images_control, dim=0)
    images_pred_all = {}
    ...
    images_control_grid = make_grid(images_control_all, nrow=nrow, padding=0, value_range=(0, 1))
    images_pred_grid = {}
    ...
    save_image(images_control_grid, os.path.join(save_dir, f"{global_step}-{name}-control.jpg"))
...
def main(
    ...
    if cfg.pretrained_unet_path is None:
        ...
        # print(unet.conv_in.state_dict()['weight'].shape)  # torch.Size([320, 8, 3, 3])
    if cfg.use_ema:
    ...
    # freeze original unet and make controlnet trainable
    unet.requires_grad_(False)
    controlnet.train()

    # train specific modules in unet
    ...
    unet.to(accelerator.device, dtype=weight_dtype)  # train controlnet only
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with (accelerator.accumulate(controlnet)):
                # (B, Nv, 3, H, W)
                imgs_in, colors_out, normals_out = batch['imgs_in'], batch['imgs_out'], batch['normals_out']
                smplx_normals = batch['normals_smplx']
                ...
                imgs_out = torch.cat([normals_out, colors_out], dim=0)
                smplx_normals_control = torch.cat([smplx_normals] * 2, dim=0)
                ...
                smplx_normals_control = rearrange(smplx_normals_control, "B Nv C H W -> (B Nv) C H W")
                ...
                imgs_in, imgs_out, camera_task_embeddings = imgs_in.to(weight_dtype), imgs_out.to(
                    weight_dtype), camera_task_embeddings.to(weight_dtype)
                smplx_normals_control.to(weight_dtype)
                ...
                down_block_res_samples, mid_block_res_sample = controlnet(
                    ...
                    controlnet_cond=smplx_normals_control,
                    return_dict=False,
